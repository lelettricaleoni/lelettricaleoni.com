...
function VideoThumbAutoplay({ hlsUrl }: { hlsUrl: string }) {
  ...
  // first frame decodes, so the placeholder sits on top and clears on real
  ...
  if (error) {
    ...
    // so this uses the same still placeholder as a route with no media at
    // all, not a loader that falsely promises it's about to come through.
    return (
  ...
  return (
    ...
    >
      {item.mediaType === 'video' ? (
        autoplay ? (
          <VideoThumbAutoplay hlsUrl={item.hlsUrl ?? hlsUrl(item.storageKey)} />
        ) : (
...
export function MediaGallery({
  ...
  const slides: Slide[] = media.map((m) =>
    m.mediaType === 'video'
      ...
      : { src: photoUrl(m.storageKey), alt: m.altText ?? `${title}` }
  )
